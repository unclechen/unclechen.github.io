...
注意，这两个App对于用户都是**始终可见**的，当它们处于`paused`状态时，也将比那些后台的处于**不可见的**App得到更高系统优先级。这个优先级怎么体现呢？假如我一直按`back`返回，当这两个其中的一个App的task返回栈已经为空时，那么系统的`back`返回按钮将开始响应屏幕上的另外一个可见的App。这就是我对上面那段话的实践和理解。
