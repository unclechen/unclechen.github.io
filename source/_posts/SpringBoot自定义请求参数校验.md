...
- 自定义校验注解：包含默认的`code`、`message`属性。
- 自定义`Validator`：在`isValid`方法中实现校验，并抛出一个自定义的异常`ValiationException`，这个异常包含了注解默认的`code`和`message`。
- 自定义异常处理方法：在捕获`ValiationException`时，取出`code`和`message`，以`ServiceResponse`返回。
